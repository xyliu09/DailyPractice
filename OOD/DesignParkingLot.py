...
class ParkingSpotType(Enum):
    HANDICAPPED, COMPACT, LARGE, MOTORBIKE, ELECTIRC = 1, 2, 3, 4, 5
...
class ParkingFloor:
    ...
    def add_parking_spot(self, spot):
        self.__handicapped_spots.update({spot.get_number(): spot})
        self.__compact_spots.update({spot.get_number(): spot})
        self.__large_spots.update({spot.get_number(): spot})
        self.__motorbike_spots.update({spot.get_number(): spot})
        self.__electric_spots.update({spot.get_number(): spot})
        switcher = {
            ParkingSpotType.HANDICAPPED: self.__handicapped_spots,
            ParkingSpotType.COMPACT: self.__compact_spots,
            ParkingSpotType.LARGE: self.__large_spots,
            ParkingSpotType.MOTORBIKE: self.__motorbike_spots,
            ParkingSpotType.ELECTRIC: self.__electric_spots,
        }
